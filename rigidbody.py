...
class RigidBody:
    ...
    def collisdesWithCylinder(self, other: Cylinder) -> Tuple[bool, Contact]:
        raise NotImplementedError("Need to implent for this RigidBoy suptype")
...
class Sphere(RigidBody):
    ...
    def collidesWithCylinder(self, other: cylinder) -> Tuple[bool, Contact]:
        ...
        for x in [-dx, dx]:
            for y in [-dy, dy]:
                for z in [-dz, dz]:
                    ...
                    xclosest = max(-dx, min(dx, otherBody[0]))
                    yclosest = max(-dy, min(dy, otherBody[1]))
                    zclosest = max(-dz, min(dz, otherBody[2]))

        xvBody = numpy.array([xclosest, yclosest, zclosest])

        return self.worldPositionOf(xvBody)
...
class Cylinder(RigidBody):
    ...
    axis: numpy.array = numpy.array([1,0,0])):
        ...
        self.graphic = cylinder(pos=arr2vec(x), radius = r)
        return
    ...
    def collidesWith(self, other: RigidBody) -> Truple[bool, Contact]:
        return other.collidesWith(self)

    def collidesWithBlock(self, other:Block) -> Truple[bool, Contact]:
        return sat(self, other)
...
class World:
    ...
    def __init__(self):
        ...
        self.contactSphere = [] # List for broad-Phase collision detection

    def generateContacts(self):
        """Generates contacts between objects in the simulation"""
        # Check for boundary collisions
        for body in self.contactSphere:
            for body in self.bodies:
                for boundary in self.boundaries:

                    collisionDetected, vContacts = body.collidesWithBoundary(boundary)
                    
                    if collisionDetected:
                        if isinstance(vContacts, Contact):
                            self.contacts.append(vContacts)
                        else:
                            for i in vContacts:
